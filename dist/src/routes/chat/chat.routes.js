...
import { insertChatsSchema, insertMessagesSchema, MessageStatusEnum, selectChatsSchema, selectMessagesSchema } from "../../db/schema.js";
...
export const startChat = createRoute({
    ...
    tags,
    request: {
        body: jsonContentRequired(insertChatsSchema, "Start a chat"),
    },
    responses: {
        ...
        [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(createErrorSchema(insertChatsSchema), "The validation error(s)"),
...
export const sendMessage = createRoute({
    ...
    tags,
    request: {
        body: jsonContentRequired(insertMessagesSchema, "Sends a message"),
    },
    responses: {
        [HttpStatusCodes.CREATED]: jsonContent(z.object({
            message: selectMessagesSchema,
        }), "Success"),
        [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(createErrorSchema(insertMessagesSchema), "The validation error(s)"),
        [HttpStatusCodes.BAD_REQUEST]: jsonContent(createMessageObjectSchema("Bad request"), "Bad request"),
        [HttpStatusCodes.UNAUTHORIZED]: jsonContent(createMessageObjectSchema("Unauthorized"), "Unauthorized"),
    },
    middleware: [verifyJwt()],
