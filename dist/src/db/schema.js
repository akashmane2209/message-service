...
export const insertChatsSchema = createInsertSchema(chats).extend({ recipients: z.array(z.string().uuid()) }).required({
    recipients: true,
}).omit({
    id: true,
    createdAt: true,
...
export const insertMessagesSchema = createInsertSchema(messages).extend({ type: MessageTypeEnum, status: MessageStatusEnum }).required({
    chatId: true,
    sender: true,
    content: true,
}).omit({
