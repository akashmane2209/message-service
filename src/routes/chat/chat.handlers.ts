import { arrayContains, count, eq, inArray, or, sql } from "drizzle-orm";
import * as HttpStatusCodes from "stoker/http-status-codes";

import type { AppRouteHandler } from "@/lib/types";

import db from "@/db";
import { chats, messages, MessageStatusValues, users } from "@/db/schema";

import type { GetChatHistoryRoute, GetChatMetaDataRoute, ListUserChatsRoute, SendMessageRoute, StartChatRoute, UpdateMessageStatusRoute } from "./chat.routes";

export const startChat: AppRouteHandler<StartChatRoute> = async (c) => {
...
export const updateMessageStatus: AppRouteHandler<UpdateMessageStatusRoute> = async (c) => {
  ...
  const nextStatusIdx = MessageStatusValues.indexOf(message.status as string) + 1;
...
export const listUserChats: AppRouteHandler<ListUserChatsRoute> = async (c) => {
  const { id } = c.req.valid("param");
...
export const getChatMetaData: AppRouteHandler<GetChatMetaDataRoute> = async (c) => {
  const { id } = c.req.valid("param");
